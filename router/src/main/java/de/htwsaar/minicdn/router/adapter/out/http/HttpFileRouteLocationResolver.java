...
public class HttpFileRouteLocationResolver implements FileRouteLocationResolver {
    ...
    public HttpFileRouteLocationResolver(@Value("${cdn.origin.base-url:http://localhost:8080}") String originBaseUrl) {
        ...
        this.originBaseUrl = URI.create(UriUtils.ensureTrailingSlash(originBaseUrl));
    }
    ...
    public URI resolveEdgeFileLocation(EdgeNode node, String path) {
        ...
        URI edgeBaseUrl = URI.create(UriUtils.ensureTrailingSlash(node.url()));
    ...
    public URI resolveOriginFileLocation(String originBaseUrl, String path) {
        ...
        if (originBaseUrl != null && !originBaseUrl.isBlank()) {
            activeOrigin = URI.create(UriUtils.ensureTrailingSlash(originBaseUrl));
        }
    ...
    private static URI resolveFileLocation(URI baseUrl, String prefix, String path) {
        String cleanPath = PathUtils.stripLeadingSlash(path == null ? "" : path.trim());
        String relativePath = prefix + cleanPath;
        return baseUrl.resolve(PathUtils.stripLeadingSlash(relativePath));
    }
}
