...
public class EdgeHttpClient implements EdgeGateway {
    ...
    public boolean isReady(URI baseUrl, Duration timeout) {
        URI readyUri = URI.create(UriUtils.ensureTrailingSlash(baseUrl.toString()) + "api/edge/ready");
    ...
    private static URI resolve(EdgeNode node, String pathOrPathAndQuery) {
        URI base = URI.create(UriUtils.ensureTrailingSlash(node.url()));
        return base.resolve(PathUtils.stripLeadingSlash(pathOrPathAndQuery));
    }
